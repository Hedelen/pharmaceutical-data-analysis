# Project Resources

A running reference list for the APIs, documentation, and libraries used in this project.

## Python / HTTP

- [Requests documentation](https://requests.readthedocs.io/en/latest/)
- [Requests quickstart](https://requests.readthedocs.io/en/latest/user/quickstart/)
  

## openFDA

- [openFDA API home](https://open.fda.gov/apis/)
- [Drug Adverse Event API](https://open.fda.gov/apis/drug/event/)
- [How to use the Drug Adverse Event endpoint](https://open.fda.gov/apis/drug/event/how-to-use-the-endpoint/)
- [Drug Adverse Event searchable fields](https://open.fda.gov/apis/drug/event/searchable-fields/)
- [Drug Adverse Event example API queries](https://open.fda.gov/apis/drug/event/example-api-queries/)
- [Understanding Drug Adverse Event API results](https://open.fda.gov/apis/drug/event/understanding-the-api-results/)
- [openFDA advanced query syntax](https://open.fda.gov/apis/advanced-syntax/)
- [openFDA API authentication / API keys](https://open.fda.gov/apis/authentication/)
- [Field Guide](https://docs.google.com/spreadsheets/d/17yJV33hIh8028ZVIIqNTCGon3JncJlvL/edit?usp=sharing&ouid=108288156109218614958&rtpof=true&sd=true)

## ChEMBL

- [ChEMBL home](https://www.ebi.ac.uk/chembl/)
- [ChEMBL interface documentation](https://chembl.gitbook.io/chembl-interface-documentation/)
- [ChEMBL web services](https://chembl.gitbook.io/chembl-interface-documentation/web-services)
- [ChEMBL programmatic access tutorial](https://www.ebi.ac.uk/training/online/courses/chembl-quick-tour/accessing-chembl-data/programmatic-access-via-web-services/)

## Notes

This file is meant to grow with the project. Add useful documentation, tutorials, API references, database dictionaries, and methodology references as they become relevant.
